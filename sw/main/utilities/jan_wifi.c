...
esp_netif_t *wifi_init_softap(void)
{
    ...
    esp_netif_set_ip4_addr(&ip_info.ip, 192, 168, 32, 1);
    esp_netif_set_ip4_addr(&ip_info.gw, 192, 168, 32, 1);
    ...
    ESP_LOGI(TAG, "WiFi softAP created. SSID:%s password:%s channel:%" PRIu16 "",
...
static void ip_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    // AP EVENTS
    {
        ...
        ESP_LOGI(TAG, "station " MACSTR " join, AID=%" PRIu8 "",
                 MAC2STR(event->mac), event->aid);
    }
    else if (event_id == WIFI_EVENT_AP_STADISCONNECTED)
    {
        wifi_event_ap_stadisconnected_t *event = (wifi_event_ap_stadisconnected_t *)event_data;
        ESP_LOGI(TAG, "station " MACSTR " leave, AID=%" PRIu8 "",
                 MAC2STR(event->mac), event->aid);
    }
}